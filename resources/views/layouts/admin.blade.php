        ...
        <div class="flex flex-col flex-1 min-h-0">
            ...
            <div class="flex items-center h-16 flex-shrink-0 px-4 bg-slate-950 border-b border-slate-800">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3">
                    ...
                    <span class="text-white font-bold text-lg tracking-wider font-sans">CONSTRUCTION<span class="text-[#36a1b3]">360</span></span>
                </a>
            </div>

            <!-- Sidebar Navigation -->
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.dashboard') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.content.edit') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.content.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.services.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.services.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.blogs.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.blogs.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.projects.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.projects.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.team.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.team.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.partners.index') }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.partners.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="mr-3 h-5 w-5 {{ request()->routeIs('admin.partners.*') ? 'text-white' : 'text-slate-400 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                    </svg>
                    Partners CRUD
                </a>

                <a href="{{ route('admin.queries.index') }}" class="flex items-center justify-between px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('admin.queries.*') ? 'bg-[#36a1b3] text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
            ...
            <div class="flex-shrink-0 p-4 bg-slate-950 border-t border-slate-850">
                <div class="flex items-center space-x-3">
                    <div class="bg-[#36a1b3] h-9 w-9 rounded-full flex items-center justify-center text-white font-semibold shadow-inner">
    ...
    <div class="md:pl-64 flex flex-col flex-1 w-full min-w-0">
        ...
        <header class="flex items-center justify-between h-16 bg-white border-b border-slate-200 px-4 md:px-8 z-10">
            ...
            <div class="flex items-center space-x-4">
                <a href="{{ route('landing') }}" target="_blank" class="hidden sm:inline-flex items-center text-xs font-semibold text-slate-600 hover:text-[#36a1b3] transition-colors">
                ...
                <div class="relative inline-block text-left" id="user-dropdown-container">
                    <button type="button" id="user-dropdown-btn" class="flex items-center space-x-2 focus:outline-none p-1.5 rounded-lg hover:bg-slate-50 transition-colors">
                        <div class="bg-[#36a1b3] h-8 w-8 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                            {{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}
                        </div>
            ...
            <div class="relative flex-1 flex flex-col max-w-xs w-full bg-slate-900 border-r border-slate-800 transition-transform">
                ...
                <div class="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
                    <div class="flex-shrink-0 flex items-center px-4">
                        <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3">
                            ...
                            <span class="text-white font-bold text-lg tracking-wider">CONSTRUCTION<span class="text-[#36a1b3]">360</span></span>
                        </a>
                    </div>
                    <nav class="mt-5 px-2 space-y-1">
                        ...
                        <a href="{{ route('admin.partners.index') }}" class="flex items-center px-3 py-2 text-base font-medium rounded-md {{ request()->routeIs('admin.partners.*') ? 'text-white bg-slate-800' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                            Partners CRUD
                        </a>
                ...
                <div class="flex-shrink-0 flex border-t border-slate-850 p-4 bg-slate-950">
                    <div class="flex items-center">
                        <div class="bg-[#36a1b3] h-9 w-9 rounded-full flex items-center justify-center text-white font-semibold">
                            {{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}
                        </div>
                        <div class="ml-3">
