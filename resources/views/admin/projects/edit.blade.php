...
<div class="max-w-3xl mx-auto">
    <div class="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-200 bg-slate-50">
            <h3 class="text-lg font-bold text-slate-900 font-sans">Modify Project</h3>
            <p class="mt-1 text-sm text-slate-500">Edit the details, images, or metadata of the project.</p>
        </div>
        ...
        <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="sm:col-span-2">
                    ...
                    <div class="mt-1.5">
                        <input type="text" name="title" id="title" value="{{ old('title', $project->title) }}" required placeholder="e.g. London Office Workspace Fit-out"
                ...
                <div>
                    ...
                    <div class="mt-1.5">
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $project->slug) }}" placeholder="e.g. london-office-fit-out"
                            class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                    </div>
                ...
                <div>
                    ...
                    <div class="mt-1.5">
                        <input type="text" name="category" id="category" value="{{ old('category', $project->category) }}" required placeholder="e.g. Commercial, Residential, Glazing"
                            class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                    </div>
                    @error('category')
                        <p class="mt-1 text-xs text-red-650">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-1.5">
                        <input type="text" name="location" id="location" value="{{ old('location', $project->location) }}" placeholder="e.g. London, Essex"
                            class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                    </div>
                    @error('location')
                        <p class="mt-1 text-xs text-red-650">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-1.5">
                        <input type="text" name="year" id="year" value="{{ old('year', $project->year) }}" placeholder="e.g. 2025"
                            class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                    </div>
                    @error('year')
                        <p class="mt-1 text-xs text-red-650">{{ $message }}</p>
            ...
            <div class="pt-4 border-t border-slate-200">
                <label class="block text-sm font-semibold text-slate-700 mb-3">Project Image</label>
                <div class="flex flex-col sm:flex-row gap-6 items-start">
                    <div class="h-28 w-44 rounded-lg overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center flex-shrink-0">
                        @if($project->image_url)
                            <img src="{{ asset($project->image_url) }}" alt="Preview" class="object-cover h-full w-full">
                        @else
                    ...
                    <div class="flex-grow space-y-4 w-full">
                        <div>
                            <label for="image" class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Replace Image File</label>
                        ...
                        <div>
                            <label for="image_url" class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Or Image Path / URL</label>
                            <input type="text" name="image_url" id="image_url" value="{{ old('image_url', $project->image_url) }}" placeholder="e.g. images/project_custom.png"
                                class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                        </div>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="mt-1.5">
                    <textarea rows="6" name="description" id="description" required placeholder="Provide a detailed description..."
                        class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">{{ old('description', $project->description) }}</textarea>
                </div>
                @error('description')
                    <p class="mt-1 text-xs text-red-650">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-4 pt-4 border-t border-slate-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-1.5">
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $project->meta_title) }}" placeholder="e.g. Modern Office Workspace Build | Construction 360"
                                class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                        </div>
                        @error('meta_title')
                            <p class="mt-1 text-xs text-red-650">{{ $message }}</p>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-1.5">
                            <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $project->meta_keywords) }}" placeholder="e.g. office build, office fit-out, corporate construction"
                                class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">
                        </div>
                        @error('meta_keywords')
                            <p class="mt-1 text-xs text-red-650">{{ $message }}</p>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-1.5">
                            <textarea rows="3" name="meta_description" id="meta_description" placeholder="A brief summary of the project for search results..."
                                class="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#008080] focus:border-transparent text-sm transition-all">{{ old('meta_description', $project->meta_description) }}</textarea>
                        </div>
